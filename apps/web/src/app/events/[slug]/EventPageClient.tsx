...
export default function EventPageClient() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Link href="/events" className="inline-flex items-center gap-2 text-brand-muted hover:text-white text-sm mb-8 transition-colors">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="card overflow-hidden">
            <div className="relative h-64 sm:h-80 bg-gradient-to-br from-purple-900 via-brand-card to-brand-dark">
              {show.posterUrl ? (
                <img src={show.posterUrl} alt={show.title} className="w-full h-full object-cover opacity-70" />
              ) : (
              ...
              <div className="absolute bottom-6 left-6">
                <h1 className="text-3xl sm:text-4xl font-extrabold text-white drop-shadow-lg">{show.title}</h1>
              </div>
            </div>
            <div className="p-6">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white text-sm font-semibold">{formatDate(show.showDate, 'EEE, d MMM yyyy')}</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white text-sm font-semibold">{show.doorsTime || formatTime(show.showDate)}</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white text-sm font-semibold">{show.venue}</p>
              ...
              {!isPast && (
                <div className="bg-brand-dark rounded-2xl p-5 border border-brand-border">
                  <p className="text-brand-muted text-xs font-semibold uppercase tracking-wider mb-3">Event Starts In</p>
          ...
          {show.description && (
            <div className="card p-6">
              ...
              <p className="text-brand-muted leading-relaxed whitespace-pre-wrap">{show.description}</p>
            </div>
          )}
        </div>
          ...
          <div className="card p-6 sticky top-24">
            ...
            <div className="space-y-3 mb-6">
              {show.ticketTypes.map((tt) => {
                const { label, variant } = getAvailabilityInfo(tt.totalQty, tt.soldQty);
                return (
                  <div key={tt.id} className="flex items-center justify-between p-4 bg-brand-dark rounded-xl border border-brand-border">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        {tt.color && <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: tt.color }} />}
                        <p className="text-white font-semibold text-sm">{tt.name}</p>
                      </div>
                      <span className={variant === 'soldout' ? 'badge-soldout' : variant === 'limited' ? 'badge-limited' : 'badge-available'}>
                        {label}
                      </span>
                    </div>
            ...
            {canBuy && (
              <Link href={`/checkout/${show.slug}`} className="btn-gold w-full justify-center text-base py-3.5">
