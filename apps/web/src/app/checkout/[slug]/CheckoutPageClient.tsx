...
export default function CheckoutPageClient() {
  ...
  const subtotal = show
    ? show.ticketTypes.reduce((sum, tt) => sum + Number(tt.price) * (quantities[tt.id] || 0), 0)
    : 0;

  const totalTickets = Object.values(quantities).reduce((s, v) => s + v, 0);

  const onSubmit = async (data: FormData) => {
    if (orderItems.length === 0) { setError('Please select at least one ticket.'); return; }
    setError('');
    setSubmitting(true);
    try {
      const result = await createOrder({
        showId: show!.id,
        items: orderItems,
        customer: { firstName: data.firstName, lastName: data.lastName, email: data.email, phone: data.phone, country: data.country, company: data.company },
              ...
              <>
                ...
                <div className="flex gap-3">
                  ...
                  <button type="submit" disabled={submitting} className="btn-gold flex-1 justify-center">
                    {submitting ? <Loader2 size={18} className="animate-spin" /> : `Pay Now — ${formatCurrency(subtotal)}`}
                  </button>
                </div>
              </>
            )}
                ...
                <div className="space-y-3 mb-4">
                  {show.ticketTypes.filter((tt) => (quantities[tt.id] || 0) > 0).map((tt) => (
                    <div key={tt.id} className="flex justify-between text-sm">
                      ...
                      <span className="text-white font-semibold">{formatCurrency(Number(tt.price) * quantities[tt.id])}</span>
                    </div>
                  ))}
                </div>
              )}
